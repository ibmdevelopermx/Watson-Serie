...
class Main extends React.Component {
  ...
  getTrendData(data) {
    ...
    fetch(`/api/trending?${qs}`)
      ...
      .catch(response => {
        this.setState({
          trendError: (response.status === 429) ? 'Numero de consultas mensuales gratuitas excedido' : 'Error fetching results',
          trendLoading: false,
          trendData: null,
  ...
  fetchData(query, clearFilters) {
    ...
    fetch(`/api/search?${qs}`)
      ...
      .catch(response => {
        this.setState({
          error: (response.status === 429) ? 'Numbero de consultas mensuales excedido' : 'Error fetching results',
          loading: false,
          data: null
        });
        // eslint-disable-next-line no-console
  ...
  render() {
    ...
    const stat_items = [
      { key: 'matches', label: 'RESEÑAS', value: numMatches },
      { key: 'positive', label: 'POSITIVAS', value: numPositive },
      { key: 'neutral', label: 'NEUTRALES', value: numNeutral },
      { key: 'negative', label: 'NEGATIVAS', value: numNegative }
    ];
